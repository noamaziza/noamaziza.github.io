...
function afficherTableau(){
    ...
    for(var i=0; i < tabJeu.length ; i++){
        ...
        for(var j=0; j < tabJeu[i].length ; j++){
            if(tabJeu[i][j] === 0){
                txt +="<img class='uneCarte' src='../asset/assetMemory/dosbleu.png' style='width:13%;height:30%;margin:3%' onClick='verif(\""+i+"-"+j+"\")'>";
            } 
            else{
                txt += "<img class='uneCarteSelect' src='"+getImage(tabJeu[i][j])+"' style=width:13%;height:30%;margin:3%>";
            } 
        }
        txt += "</div>";
    }
